...
export const HomePage: React.FC = () => {
  ...
  // Helper to extract shows for a section regardless of list vs record format
  const getShowsForSection = (sectionKey: string): CatalogueShow[] => {
    if (!catalog?.sections) return [];
    if (Array.isArray(catalog.sections)) {
      const found = catalog.sections.find((s) => s.section === sectionKey);
      return found?.shows || [];
    }
    return (catalog.sections as Record<string, CatalogueShow[]>)[sectionKey] || [];
  };

  const featuredShows = getShowsForSection("featured");
  const allShows: CatalogueShow[] = Array.isArray(catalog.sections)
    ? catalog.sections.flatMap((s) => s.shows || [])
    : Object.values(catalog.sections || {}).flat();

  const heroShow: CatalogueShow | undefined =
    catalog.featured || featuredShows[0] || allShows[0] || catalog.shows?.[0];
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-10">
      ...
      <div className="space-y-8">
        {REFERENCE_SECTIONS.map((sec) => {
          const sectionShows = getShowsForSection(sec.key);
          return (
            <ShowRow
              key={sec.key}
              title={sec.title}
