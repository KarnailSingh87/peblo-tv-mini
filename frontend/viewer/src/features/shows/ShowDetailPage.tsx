...
import { CatalogueEpisodeGroup, CatalogueShow, CatalogueEpisodeVariant } from "../../types";
...
export const ShowDetailPage: React.FC = () => {
  ...
  const allShows: CatalogueShow[] = catalog?.sections
    ? Array.isArray(catalog.sections)
      ? catalog.sections.flatMap((s) => s.shows || [])
      : Object.values(catalog.sections).flat()
    : catalog?.shows || [];
  const show: CatalogueShow | undefined = allShows.find((s) => s.slug === slug);

  // Helper for variant and language resolution
  const getActiveVariantTitle = (ep: CatalogueEpisodeGroup, lang: string): string => {
    if (!ep) return "Episode";
    if (Array.isArray(ep.variants) && ep.variants.length > 0) {
      const match = ep.variants.find((v) => v.language === lang) || ep.variants[0];
      return match?.episode_title || ep.title || `Episode ${ep.episode_number}`;
    }
    if (ep.variants && typeof ep.variants === "object") {
      const dict = ep.variants as Record<string, CatalogueEpisodeVariant>;
      const match = dict[lang] || Object.values(dict)[0];
      return match?.episode_title || ep.title || `Episode ${ep.episode_number}`;
    }
    return ep.title || `Episode ${ep.episode_number}`;
  };

  const getEpLanguages = (ep: CatalogueEpisodeGroup): string[] => {
    if (ep.languages && ep.languages.length > 0) return ep.languages;
    if (ep.available_languages && ep.available_languages.length > 0) return ep.available_languages;
    if (Array.isArray(ep.variants)) return Array.from(new Set(ep.variants.map((v) => v.language)));
    if (ep.variants && typeof ep.variants === "object") return Object.keys(ep.variants);
    return ["en"];
  };
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {show.trailers.map((trailer) => {
              const trailerTitle = getActiveVariantTitle(trailer, preferredLang);
              const trailerLangs = getEpLanguages(trailer);
                ...
                >
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-slate-800/60 text-xs">
                    <span className="text-slate-400 text-[11px]">
                      Languages: {trailerLangs.map((l) => l.toUpperCase()).join(", ")}
                    </span>
        ...
        {currentSeason?.episodes?.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {currentSeason?.episodes?.map((epGroup: CatalogueEpisodeGroup) => {
              const epTitle = getActiveVariantTitle(epGroup, preferredLang);
              const epLangs = getEpLanguages(epGroup);
                ...
                >
                  ...
                  <div className="space-y-1.5">
                    <h4 className="font-bold text-sm text-slate-100 group-hover:text-indigo-300 transition-colors line-clamp-1">
                      {epTitle || `Episode ${epGroup.episode_number}`}
                    </h4>
