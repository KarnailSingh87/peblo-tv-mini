...
class CataloguePublisher:
    ...
    def publish(cls, db: Session, user: User) -> Tuple[PublishRun, Dict[str, Any]]:
        ...
        # 4. Persist to storage atomically
        storage = get_storage()
        try:
            # Atomic save to live catalogue.json
            storage.save_atomic(catalog_json_bytes, "catalogue.json", content_type="application/json")
            # Also save versioned archive copy
            storage.save_atomic(catalog_json_bytes, f"catalogue_v{next_version}.json", content_type="application/json")
        except Exception as e:
            run.completed_at = datetime.utcnow()
            run.status = "failed"
            run.error_message = f"Storage write error during atomic write: {str(e)}"
            db.commit()
